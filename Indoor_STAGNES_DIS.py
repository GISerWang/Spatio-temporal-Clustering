...
def plotFeature(data, clusterArr):
    ...
    scatterColors = ['red', 'blue', 'orange', 'orchid', 'black', 'green', 'brown', 'mediumpurple', '#E6E0E4', '#E6E0E4']
...
def getClosestCluster(clusterTimeMat,clusterDisMat,timeThreh,clusterFloorIds):
    ...
    # tileA与tileA是一个技巧的使用
    # np.tile(clusterFloorIds, (n, 1)) 将clusterFloorIds列复制n次
    # np.tile(clusterFloorIds.reshape(-1,1), (1,n)) 首先将数组转置，然后横向复制N次
    # tileA == tileB表示，每i行都应该等于数组中的第i个数组，即clusterFloorIds == clusterFloorIds[i]
    tileA = np.tile(clusterFloorIds, (n, 1))
    tileB = np.tile(clusterFloorIds.reshape(-1,1), (1,n))
    # 满足两个条件：时间阈值小于timeThreh（但是不能为0），楼层ID等于clusterFloorIds[i]
    disA = np.where((clusterTimeMat < timeThreh) & (clusterTimeMat != 0) & (tileA == tileB), clusterDisMat, np.inf)
    # 返回的最近的簇集ID，即最小距离
    return np.argmin(disA, axis=1), np.min(disA, axis=1)
# data 的第一列是unix时间戳，第二、三列是空间X,Y坐标，第四列是楼层ID
# disThreh 空间阈值
# timeThreh 时间阈值
def Indoor_STAGNES(data,disThreh,timeThreh):
    # 获得数据的行和列(一共有n条数据)
    n, m = data.shape
    # 获得每一个轨迹点的时间数组
    timeArr=data[:,0]
    ...
    # 求得最小距离
    minDis = np.min(closedClusterDis)
    while minDis < disThreh:
        # 获得需要合并的簇集ID
        ...
        # clusterFloorIds为ndarray，采用np.delete删除
...
start = time.clock()
clusterArr = Indoor_STAGNES(data,6,30)
end = time.clock()
